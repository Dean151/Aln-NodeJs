...
class DataBaseCoordinator {
  ...
  getUserBy(column, value, callback) {
    ...
    this.con.query('SELECT u.*, GROUP_CONCAT(CONCAT(f.identifier, \':\', f.name)) as feeders FROM users u LEFT JOIN feeders f ON f.owner = u.id WHERE u.' + column + ' = ?', [value], (err, results, fields) => {
      if (err) { throw err; }
